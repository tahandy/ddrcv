...
class RemyParser:
    ...
    def load_page(self):
        """Loads the webpage and initializes the BeautifulSoup object."""
        try:
    ...
    def scrape_song_list(self):
        """Scrapes the first <a> link from each <li> within <ul> elements that are immediate children of the div with class 'mw-parser-output'."""
        if not self.soup:
    ...
    def is_song_processed(self, song_dir):
        """Checks if a song is already processed by verifying PNG files and JSON fields."""
        try:
    ...
    def process_song(self, link_data):
        """Process a link to extract and download images, and parse metadata."""
        link, list_item_text = link_data
        song_dir = self.output_dir / self.sanitize_filename(list_item_text)
    ...
    def extract_field(self, paragraph, field_name):
        """Helper method to extract a specific field from a paragraph."""
    ...
    def process_images(self, content_div, list_item_text):
        """Extract images from the mw-parser-output div and download them."""
    ...
    def get_full_image_url(self, img_src):
        """Converts the thumbnail image src to the full image URL."""
    ...
    def download_image(self, img_url, list_item_text):
        ...
        try:
            ...
            item_dir = self.output_dir / self.sanitize_filename(list_item_text)
    ...
    def extract_table(self, content_div):
        """Extract the specific table data from the mw-parser-output div."""
    ...
    def parse_table(self, table):
        """Parse the table to extract relevant data."""
        table_data = []
    ...
    def extract_ddr_value(field_value, game="DanceDanceRevolution"):
        """
        Extracts the value associated with DanceDanceRevolution from a string containing multiple game variants
        or returns the value if no games are specified.

        Args:
            field_value (str): The full string containing game-specific variants.
            game (str): The game to search for. Default is "DanceDanceRevolution".

        Returns:
            str: The value associated with the specified game, or the entire value if no games are specified.
        """
...
def main():
    config = load_config('configs/ddr_world.json')
    parser = RemyParser(config)
    list_items = parser.scrape_song_list()

    # Process the links in parallel with rate limiting
    parser.process_links_in_parallel(list_items)

...
from ddrcv.ocr import get_best_match_from_results
...
def parse_scores(parser, image, config, box_offset, upsample=1, do_blur=False, do_invert=True, padding=0):
    ...
    # debug = np.zeros((chips.shape[0] * chips.shape[1], chips.shape[2], chips.shape[3]), dtype=chips.dtype)
    # for ii, chip in enumerate(chips):
    #     debug[ii * chips.shape[1]:(ii+1) * chips.shape[1], ...] = chip
    # cv2.imshow('tmp', debug)
    # cv2.waitKey(0)
...
class ResultsParser:
    def __init__(self, ocr_parser, database):
        ...
        self.p1 = PlayerParser(self.parser, p1_config)
        self.p2 = PlayerParser(self.parser, p2_config)
...
if __name__ == "__main__":
    ...
    # parse_scores(reader, img, score_box_config, p2_config['score_topleft'], upsample=3, do_blur=True, do_invert=True, padding=4)

    # tic = time.time()
    # p2 = PlayerParser(reader, p2_config)
    # pprint(p2.parse(img))
    # print(f'Parsing P2 took {time.time() - tic:.3f} seconds')
    #
    # tic = time.time()
    # p1 = PlayerParser(reader, p1_config)
    # pprint(p1.parse(img))
    # print(f'Parsing P1 took {time.time() - tic:.3f} seconds')
    ...
    #
    #
    # score_tl = p2_config['score_topleft']
